...
public class UserProductTypeDTO extends SuperDTO{
	...
	public List<UserProductType> getUserProductTypes(Integer id, Date startDate, Date endDate) throws SQLException, IllegalAccessException, InstantiationException {
		PreparedStatement statement = connection.prepareStatement(SELECT_PRODUCT_TYPES);
		statement.setInt(1, id);
		statement.setDate(2, startDate);
		statement.setDate(3, endDate);
		ResultSet resultSet = statement.executeQuery();
		List<UserProductType> productTypes = new Transformer<UserProductType>(UserProductType.class).fromRStoCollection(resultSet);
		statement.close();
		return productTypes;
	}
}
