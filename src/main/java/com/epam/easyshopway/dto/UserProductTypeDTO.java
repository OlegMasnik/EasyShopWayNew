...
public class UserProductTypeDTO extends SuperDTO{
	private final String SELECT_PRODUCT_TYPES = "SELECT pt.name_en, pt.name_uk, COUNT(*) AS count FROM product_list pl JOIN product p ON pl.product_id = p.id JOIN product_type pt ON p.product_type_id = pt.id WHERE pl.user_id = ? GROUP BY pt.name_en ORDER BY pt.name_en ASC;";
