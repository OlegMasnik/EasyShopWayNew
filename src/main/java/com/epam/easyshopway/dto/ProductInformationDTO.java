...
public class ProductInformationDTO extends SuperDTO {
	...
	private final String SELECT_ALL_PRODUCTS_BY_MAP_ID = "SELECT DISTINCT product.id, product.name_en, product.name_uk, product_type.img, placement.map_id "
			...
			+ "INNER JOIN product_placement ON product_placement.cupboard_id = cupboard.id "
			+ "INNER JOIN product ON product_placement.product_id = product.id "
			+ "INNER JOIN product_type ON product.product_type_id = product_type.id " + "where placement.map_id like ?";
	...
	public List<ProductInformation> getAllProduct()
		...
		List<ProductInformation> users = new Transformer<ProductInformation>(ProductInformation.class)
	...
	public List<ProductInformation> getAllProductByMapId(Integer mapId)
			throws SQLException, InstantiationException, IllegalAccessException {
		PreparedStatement statement = connection.prepareStatement(SELECT_ALL_PRODUCTS_BY_MAP_ID);
	...
	public List<ProductInformation> getAllProductbyProductListId(Integer productListId)
			throws SQLException, InstantiationException, IllegalAccessException {
		PreparedStatement statement = connection.prepareStatement(SELECT_PRODUCTS_BY_USER_LIST_ID);
	...
	public List<ProductInformation> getAllProductbyCupdoardId(Integer productListId)
			throws SQLException, InstantiationException, IllegalAccessException {
		PreparedStatement statement = connection.prepareStatement(SELECT_PRODUCTS_BY_CUPBOARD_ID);
		statement.setInt(1, productListId);
		ResultSet resultSet = statement.executeQuery();
		List<ProductInformation> users = new Transformer<ProductInformation>(ProductInformation.class)
				.fromRStoCollection(resultSet);
