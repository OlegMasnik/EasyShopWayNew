...
public class CupboardInformationDTO extends SuperDTO{
	...
	private final String GET_CUPBOARDS = "SELECT p.map_id,  cp.cupboard_id,  p.place, c.description_en, c.description_uk, c.board_amount FROM placement p JOIN cupboard_placement cp ON p.id = cp.placement_id JOIN cupboard c ON cp.cupboard_id = c.id WHERE p.map_id = ? AND c.active = 1 ORDER BY cp.cupboard_id ASC, p.place ASC;";
